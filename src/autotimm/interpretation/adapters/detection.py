...
def _parse_detections(
    ...
    if isinstance(detections_output, dict):
        ...
        scores = detections_output.get('scores', detections_output.get('confidence', None))
        labels = detections_output.get('labels', detections_output.get('class_id', None))
...
def _find_closest_detection(
    ...
    for det in detections:
        ...
        dist = np.sqrt((center[0] - target_center[0])**2 + (center[1] - target_center[1])**2)
...
def _visualize_detection_explanations(
    ...
    # Create figure
    fig, axes = plt.subplots(1, min(4, len(detections) + 1), figsize=(4 * min(4, len(detections) + 1), 4))
    if len(detections) == 0:
        axes = [axes]
