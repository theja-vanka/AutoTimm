...
class MultiLabelImageDataModule(pl.LightningDataModule):
    ...
    def _print_summary(self) -> None:
        ...
        try:
            ...
            print(f"\n{'=' * 50}")
            print("MultiLabelImageDataModule Summary")
            print(f"{'=' * 50}")
            print(f"Image dir: {self.image_dir}")
            print(f"Image size: {self.image_size}")
            print(f"Batch size: {self.batch_size}")
            if self._num_labels is not None:
                print(f"Num labels: {self._num_labels}")
            if self._label_names is not None:
                print(f"Labels: {', '.join(self._label_names)}")
    ...
    def summary(self):
        ...
        Call after ``setup()`` so that datasets and label info are available.
        """
        ...
        table.add_row("Image dir", str(self.image_dir))
        table.add_row("Train CSV", str(self.train_csv))
        if self.val_csv is not None:
            table.add_row("Val CSV", str(self.val_csv))
        if self.test_csv is not None:
            table.add_row("Test CSV", str(self.test_csv))
        ...
        if self._num_labels is not None:
            table.add_row("Num labels", str(self._num_labels))
        if self._label_names is not None:
            table.add_row("Label names", ", ".join(self._label_names))
